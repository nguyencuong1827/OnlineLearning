import {Colors} from './';

const themes = {
  light: {
    name: 'light',
    backgroundColor: Colors.white95,
    colorMainText: Colors.black,
    colorSubText: Colors.white40,
    colorIconTab: Colors.white47,
    colorIconActiveTab: Colors.blue36,
    headerFooterBackground: Colors.white,
    buttonSeeAllBackground: Colors.white85,
    searchBarBackground: Colors.white52,
  },
  dark: {
    name: 'dark',
    backgroundColor: Colors.black5,
    colorMainText: Colors.white95,
    colorSubText: Colors.white52,
    colorIconTab: Colors.white52,
    colorIconActiveTab: Colors.blue50,
    headerFooterBackground: Colors.black14,
    buttonSeeAllBackground: Colors.black14,
    searchBarBackground: Colors.white40,
  },
  system: {
    name: 'system',
    backgroundColor: Colors.white95,
    colorMainText: Colors.black,
    colorSubText: Colors.white40,
    colorIconTab: Colors.white47,
    colorIconActiveTab: Colors.blue50,
    headerFooterBackground: Colors.white,
    buttonSeeAllBackground: Colors.white85,
    searchBarBackground: Colors.white52,
  },
};
export default themes;
